...
export default function CandidateRegister() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    try {
      const response = await axios.post('/api/auth/register', { ...formData, role: 'candidate' })
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center py-12">
      <div className="bg-slate-800 border border-slate-700 p-8 rounded-2xl shadow-lg max-w-md w-full">
        <div className="text-center mb-8">
          ...
          <p className="text-blue-300 font-semibold">Candidate Registration</p>
        </div>
        ...
        <p className="text-center mt-6 text-gray-400">
          Already have an account? <a href="/candidate-login" className="text-blue-400 hover:text-cyan-400 transition">Login</a>
        </p>
      </div>
    </div>
  )
